...
# 2) 왼쪽 방향에 아직 가보지 않은 칸이 있다면, 왼쪽으로 회전한 다음 외쪽으로 한 칸을 전진하고
#    왼쪽 방향으로 가보지 않은 칸이 없다면, 외쪽 방향으로 회전만하고 1단계로 돌아간다
# 3) 만약 제 방향이 모두 visited or sea, 바라보는 방향을 유지한 채로 한 칸 뒤로 가고 1단계로 돌아간다
